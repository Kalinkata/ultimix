...
class nusoap_base {
	...
	*/
	...
	'http://www.w3.org/2001/XMLSchema' => array(
		...
		'NCName'=>'','ID'=>'','IDREF'=>'','IDREFS'=>'','ENTITY'=>'','ENTITIES'=>'','integer'=>'integer',
		'nonPositiveInteger'=>'integer','negativeInteger'=>'integer','long'=>'integer','int'=>'integer',
    ...
    */
	...
	$this->debug("In serializeEnvelope length=" . strlen($body) . " body (max 1000 characters)=" . 
									substr($body, 0, 1000) . " style=$style use=$use encodingStyle=$encodingStyle");
	$this->debug("headers:");
...
class nusoap_xmlschema extends nusoap_base  {
		...
		switch($name){
			...
			case 'complexType':
				...
				if(isset($attrs['name'])){
					...
					$this->xdebug('processing unnamed complexType for element ' . 
																		$this->currentElement . ' named ' . $name);
			...
			case 'include':
			    if (isset($attrs['schemaLocation'])) {
					$this->xdebug('include into namespace ' . $this->schemaTargetNamespace . 
																				' from ' . $attrs['schemaLocation']);
			...
			case 'simpleType':
				...
				if(isset($attrs['name'])){
					...
					$this->xdebug('processing unnamed simpleType for element ' . 
																			$this->currentElement . ' named ' . $name);
	...
	*/
	function schemaEndElement($parser, $name) {
		...
		// move on...
		if($name == 'complexType'){
			$this->xdebug('done processing complexType ' . ($this->currentComplexType ? 
																			$this->currentComplexType : '(unknown)'));
		...
		if($name == 'simpleType'){
			$this->xdebug('done processing simpleType ' . ($this->currentSimpleType ? 
																			$this->currentSimpleType : '(unknown)'));
	...
	function serializeSchema(){
		...
		if (sizeof($this->imports) > 0) {
			foreach($this->imports as $ns => $list) {
				foreach ($list as $ii) {
					if ($ii['location'] != '') {
						$xml .= " <$schemaPrefix:import location=\"" . $ii['location'] . '" namespace="' . 
																									$ns . "\" />\n";
					} else {
		...
		foreach($this->complexTypes as $typeName => $attrs){
			...
			if (isset($attrs['restrictionBase']) && $attrs['restrictionBase'] != ''){
				...
				if ((isset($attrs['elements']) && count($attrs['elements']) > 0) || (isset($attrs['attrs']) && 
																						count($attrs['attrs']) > 0)){
		...
		if(isset($this->elements) && count($this->elements) > 0){
			foreach($this->elements as $element => $eParts){
				$xml .= " <$schemaPrefix:element name=\"$element\" type=\"".
																		$this->contractQName($eParts['type'])."\"/>\n";
			}
		}
		// attributes
		if(isset($this->attributes) && count($this->attributes) > 0){
			foreach($this->attributes as $attr => $aParts){
				$xml .= " <$schemaPrefix:attribute name=\"$attr\" type=\"".
																		$this->contractQName($aParts['type'])."\"\n/>";
			}
		}
		// finish 'er up
		$attr = '';
	...
	* @see getTypeDef
	*/
	function addComplexType($name,$typeClass='complexType',$phpType='array',$compositor='',$restrictionBase='',
...
class soapval extends nusoap_base {
	...
	*/
	function serialize($use='encoded') {
		return $this->serialize_val($this->value, $this->name, $this->type, $this->element_ns, $this->type_ns, 
																						$this->attributes, $use, true);
    }
...
class soap_transport_http extends nusoap_base {
	...
	var $certRequest = array();	// keys must be cainfofile (optional), sslcertfile, sslkeyfile, passphrase, 
								// certpassword (optional), verifypeer (optional), verifyhost (optional)
	...
	*/
	function io_method() {
		if ($this->use_curl || ($this->scheme == 'https') || ($this->scheme == 'http' && 
				$this->authtype == 'ntlm') || ($this->scheme == 'http' && is_array($this->proxy) && 
...
//		  	}
//		}
		$this->debug("connect connection_timeout $connection_timeout, response_timeout $response_timeout, ".
														"scheme $this->scheme, host $this->host, port $this->port");
...
//			$this->setError('cURL Extension, or OpenSSL extension w/ PHP version >= 4.3 is required for HTTPS');
			$this->setError('The PHP cURL Extension is required for HTTPS or NLTM.  You will need to re-build or ".
							"update your PHP to include cURL or change php.ini to load the PHP cURL extension.');
			return false;
		}
...
//		}
	...
	*	certpassword (optional), verifypeer (optional), verifyhost (optional): see corresponding options in cURL docs)
	* @access   public
	*/
		...
		if ($authtype == 'basic') {
			...
			if (isset($digestRequest['nonce'])) {
				...
				if ($digestRequest['qop'] != '') {
					$unhashedDigest = $HA1 . ':' . $nonce . ':' . sprintf("%08d", $digestRequest['nc']) . ':' . 
																	$cnonce . ':' . $digestRequest['qop'] . ':' . $HA2;
				} else {
				...
				$this->setHeader('Authorization', 'Digest username="' . $username . '", realm="' . 
					$digestRequest['realm'] . '", nonce="' . $nonce . '", uri="' . $this->digest_uri . $opaque . 
					'", cnonce="' . $cnonce . '", nc=' . sprintf("%08x", $digestRequest['nc']) . ', qop="' . 
...
//		}
		...
		if (
			...
			$this->setError("Unsupported HTTP response status $http_status ".
				"$http_reason (soapclient->response has contents of the response)");
			return false;
		}
		...
		if(isset($this->incoming_headers['content-encoding']) && $this->incoming_headers['content-encoding'] != ''){
			if(strtolower($this->incoming_headers['content-encoding']) == 'deflate' || 
    			...
    			if(function_exists('gzinflate')){
						...
						if ($degzdata = @gzinflate(substr($data, 10))) {	// do our best
	    					...
	    					if (strlen($data) < $datalen) {
								...
								if ($degzdata = @gzinflate(substr($data, 10))) {
			    					$data = $degzdata;
			    					$this->debug('The payload has been un-gzipped again to ' . 
										strlen($data) . ' bytes');
								}
	    					}
	    				} else {
					...
					$this->debug('The server sent compressed data. Your php install must ".
						"have the Zlib extension compiled in to support this.');
					$this->setError('The server sent compressed data. Your php install must ".
						"have the Zlib extension compiled in to support this.');
				}
			} else {
...
class nusoap_server extends nusoap_base {
	...
	*/
	function invoke_method() {
		$this->debug('in invoke_method, methodname=' . $this->methodname . ' methodURI=' . 
			$this->methodURI . ' SOAPAction=' . $this->SOAPAction);
		...
		if ($this->wsdl) {
			if ($this->opData = $this->wsdl->getOperationData($this->methodname)) {
				...
				// Note: hopefully this case will only be used for doc/lit, since rpc services 
				// should have wrapper element
				$this->debug('in invoke_method, found WSDL soapAction=' . $this->SOAPAction . 
					' for operation=' . $this->opData['name']);
				$this->appendDebug('opData=' . $this->varDump($this->opData));
		...
		if ($class == '') {
			if (!function_exists($this->methodname)) {
				...
				$this->fault('SOAP-ENV:Client',"method '$this->methodname'('$orig_methodname')".
					" not defined in service('$try_class' '$delim')");
				return;
			}
		} else {
			...
			if (!in_array($method_to_compare, get_class_methods($class))) {
				...
				$this->result = 'fault: method not found';
				$this->fault('SOAP-ENV:Client',"method '$this->methodname'/'$method_to_compare'('$orig_methodname')".
					" not defined in service/'$class'('$try_class' '$delim')");
				return;
			}
		}
		...
		if (!function_exists('call_user_func_array')) {
			...
			if ($this->methodparams) {
				foreach ($this->methodparams as $param) {
					if (is_array($param) || is_object($param)) {
						$this->fault('SOAP-ENV:Client', 'NuSOAP does not handle complexType parameters".
							" correctly when using eval; call_user_func_array must be available');
						return;
					}
					$funcCall .= "\"$param\",";
				}
			...
			if ($class == '') {
				...
				$call_arg = "$this->methodname";	// straight assignment changes $this->methodname to lower
				// case after call_user_func_array()
			} elseif ($delim == '..') {
		...
		$this->debug("in invoke_method, called method $this->methodname, received data of type ".
	...
	function serialize_return() {
		...
		if ($this->wsdl) {
			...
			if ($this->opData['style'] == 'rpc') {
				...
				if ($this->opData['output']['use'] == 'literal') {
					if ($this->methodURI) {
						$payload = '<ns1:'.$this->methodname.'Response xmlns:ns1="'.$this->methodURI.'">'.
							$return_val.'</ns1:'.$this->methodname."Response>";
					} else {
		...
		if($this->wsdl){
			...
			$this->responseSOAP = $this->serializeEnvelope($payload,$this->responseHeaders,
				$this->wsdl->usedNamespaces,$this->opData['style'],$this->opData['output']['use'],$encodingStyle);
		} else {
			$this->responseSOAP = $this->serializeEnvelope($payload,$this->responseHeaders);
		}
	...
	* @access   public
	*/
	function register($name,$in=array(),$out=array(),$namespace=false,$soapaction=false,$style=false,$use=false,
				$documentation='',$encodingStyle=''){
		global $HTTP_SERVER_VARS;

		if($this->externalWSDLURL){
			die('You cannot bind to an external WSDL file, and register methods outside of it! Please '.
				'choose either WSDL or no WSDL.');
		}
		if (! $name) {
		...
		if(false == $soapaction) {
			if (isset($_SERVER)) {
				...
				$HTTPS = isset($_SERVER['HTTPS']) ? $_SERVER['HTTPS'] : (isset($HTTP_SERVER_VARS['HTTPS']) ? 
    ...
    */
								...
								$transport = 'http://schemas.xmlsoap.org/soap/http', $schemaTargetNamespace = false)
    {
		...
		// If server name has port number attached then strip it (else port number gets duplicated in WSDL output)
		// (occurred using lighttpd and FastCGI)
...
class wsdl extends nusoap_base {
     ...
     * @access public 
     */
    function wsdl($wsdl = '',$proxyhost=false,$proxyport=false,$proxyusername=false,$proxypassword=false,$timeout=0,
...
isset($this->bindings[$this->currentBinding]['endpoint']) ? $this->bindings[$this->currentBinding]['endpoint'] : '';
						    } 
						    break;
						case 'input':
						    $this->opStatus = 'input';
						...
						case 'output':
						    ...
						    break;
						case 'body':
						    if (isset($this->bindings[$this->currentBinding]['operations'][
							$this->currentOperation][$this->opStatus])) {
						        $this->bindings[$this->currentBinding]['operations'][
								$this->currentOperation][$this->opStatus] = array_merge($this->bindings[
								$this->currentBinding]['operations'][$this->currentOperation][$this->opStatus], $attrs);
						    } else {
						        $this->bindings[$this->currentBinding]['operations'][$this->currentOperation][
								$this->opStatus] = $attrs;
						    } 
						    break;
					} 
	...
	*/
	function serialize($debug = 0)
	{
			...
			foreach($this->messages as $msgName => $msgParts) {
				...
				if(is_array($msgParts)){
					foreach($msgParts as $partName => $partType) {
						...
						$xml .= "\n" . '  <part name="' . $partName . '" ' . $elementortype . '="' . 
							$typePrefix . ':' . $localPart . '" />';
					}
				}
				$xml .= '</message>';
			} 
		...
		if (count($this->bindings) >= 1) {
			...
			foreach($this->bindings as $bindingName => $attrs) {
				...
				$binding_xml .= "\n" . '  <soap:binding style="' . $attrs['style'] . '" transport="' . 
					$attrs['transport'] . '"/>';
				...
				foreach($attrs['operations'] as $opName => $opParts) {
					...
					$binding_xml .= "\n" . '    <input><soap:body use="' . $opParts['input']['use'] . 
						'" namespace="' . $opParts['input']['namespace'] . '"' . $enc_style . '/></input>';
					...
					$binding_xml .= "\n" . '    <output><soap:body use="' . $opParts['output']['use'] . 
						'" namespace="' . $opParts['output']['namespace'] . '"' . $enc_style . '/></output>';
					$binding_xml .= "\n" . '  </operation>';
		...
		if (count($this->ports) >= 1) {
			foreach($this->ports as $pName => $attrs) {
				...
				$xml .= "\n" . '    <soap:address location="' . $attrs['location'] . 
					($debug ? '?debug=1' : '') . '"/>';
				$xml .= "\n" . '  </port>';
			} 
		} 
	...
	function serializeRPCParameters($operation, $direction, $parameters, $bindingType = 'soap') {
		$this->debug("in serializeRPCParameters: operation=$operation, direction=$direction, ".
	...
	{
		$this->debug("in serializeParameters: operation=$operation, direction=$direction, ".
	...
	{
		$this->debug("in serializeType: name=$name, type=$type, use=$use, encodingStyle=$encodingStyle, ".
		...
		if ($phpType == 'struct') {
			...
			if (is_null($value)) {
				if ($use == 'literal') {
					...
					$xml = "<$elementName$elementNS xsi:nil=\"true\" xsi:type=\"" . 
						$this->getPrefixFromNamespace($ns) . ":$uqType\"/>";
				}
				$this->debug("in serializeType: returning: $xml");
				return $xml;
			...
			if (is_array($value)) {
				...
				if ($use == 'literal') {
					if ($forceType) {
						$xml = "<$elementName$elementNS$elementAttrs xsi:type=\"" . 
							$this->getPrefixFromNamespace($ns) . ":$uqType\">";
					} else {
				...
				foreach($value as $k => $v) {
					...
					if (!in_array($typeDef['arrayType'],$this->typemap['http://www.w3.org/2001/XMLSchema'])) {
					    ...
					    $contents .= $this->serialize_val($v, 'item', $typeDef['arrayType'], 
							null, $this->XMLSchemaVersion, false, $use);
					} 
				}
			} else {
				$rows = 0;
		...
		if (isset($typeDef['elements']) && is_array($typeDef['elements'])) {
			...
			foreach ($typeDef['elements'] as $eName => $attrs) {
				...
				if (isset($optionals)
					...
					// do nothing
					$this->debug("no value provided for complexType element $eName and element ".
						"is not nillable, so serialize nothing");
				} else {
					// get value
					if (isset($xvalue[$eName])) {
					    $v = $xvalue[$eName];
					...
					if (isset($attrs['maxOccurs']) && ($attrs['maxOccurs'] == 'unbounded' || $attrs['maxOccurs'] > 1) 
						...
						foreach ($vv as $k => $v) {
							if (isset($attrs['type']) || isset($attrs['ref'])) {
							    ...
							    $xml .= $this->serializeType($eName, isset($attrs['type']) ? $attrs['type'] : 
									$attrs['ref'], $v, $use, $encodingStyle, $unqualified);
							} else {
								// serialize generic type (can this ever really happen?)
							    $this->debug("calling serialize_val() for $v, $eName, false, ".
	...
	*/
	function addComplexType($name,$typeClass='complexType',$phpType='array',$compositor='',$restrictionBase='',
		...
		$this->schemas[$typens][0]->addComplexType($name,$typeClass,$phpType,$compositor,$restrictionBase,
	...
	* @param string $style (rpc|document) optional The style for the operation Note: when 'document' is specified, 
	...
	* @access public 
	*/
	function addOperation($name, $in = false, $out = false, $namespace = false, $soapaction = false, $style = 'rpc', 
			$use = 'encoded', $documentation = '', $encodingStyle = ''){
		if ($use == 'encoded' && $encodingStyle == '') {
			$encodingStyle = 'http://schemas.xmlsoap.org/soap/encoding/';
		}
...
class nusoap_parser extends nusoap_base {
			...
			if ($pos_xml !== FALSE) {
				...
				if (preg_match("/encoding=[\"']([^\"']*)[\"']/", $xml_decl, $res)) {
					...
					if (strtoupper($xml_encoding) != $encoding) {
						...
						// when HTTP says ISO-8859-1 (the default) and XML says UTF-8 (the typical), assume the 
						// other endpoint is just sloppy and proceed
					} else {
		...
		if(isset($this->body_position) && $pos > $this->body_position){
			...
			// set value of simpleType (or nil complexType)
				...
				if (isset($this->message[$pos]['nil']) && $this->message[$pos]['nil']) {
					...
					isset($this->message[$parent]['arrayType'])) {
						$this->message[$pos]['result'] = $this->decodeSimple($this->message[$pos]['cdata'], 
						$this->message[$parent]['arrayType'], isset($this->message[$parent]['arrayTypeNamespace']) ? 
						$this->message[$parent]['arrayTypeNamespace'] : '');
					} else {
	...
	*/
	function buildVal($pos){
		...
		$this->debug('in buildVal() for '.$this->message[$pos]['name']."(pos $pos) of type ".
			$this->message[$pos]['type']);
		...
		if($this->message[$pos]['children'] != ''){
			...
			if (isset($this->message[$pos]['cdata']) && trim($this->message[$pos]['cdata']) != '') {
            	...
            	if (isset($this->message[$pos]['type'])) {
					$params['!'] = $this->decodeSimple($this->message[$pos]['cdata'], $this->message[$pos]['type'], 
        	...
        	if (isset($this->message[$pos]['type'])) {
				$ret = $this->decodeSimple($cdata, $this->message[$pos]['type'], 
					isset($this->message[$pos]['type_namespace']) ? $this->message[$pos]['type_namespace'] : '');
			...
			if (isset($this->message[$parent]['type']) && ($this->message[$parent]['type'] == 'array') && 
				isset($this->message[$parent]['arrayType'])) {
				$ret = $this->decodeSimple($cdata, $this->message[$parent]['arrayType'], 
					isset($this->message[$parent]['arrayTypeNamespace']) ? 
						$this->message[$parent]['arrayTypeNamespace'] : '');
				$this->debug("in buildVal, return: $ret");
				return $ret;
...
class nusoap_client extends nusoap_base  {
	...
	* @param	string	$style optional (rpc|document) the style to use when serializing parameters 
	*			(WSDL can override)
	...
	* @access   public
	*/
	function call($operation,$params=array(),$namespace='http://tempuri.org',$soapAction='',$headers=false,
		...
		$this->debug("call: operation=$operation, namespace=$namespace, soapAction=$soapAction, ".
				"rpcParams=$rpcParams, style=$style, use=$use, endpointType=$this->endpointType");
		...
		if ($style == 'rpc') {
			if ($use == 'literal') {
				...
				if ($namespace) {
					// http://www.ws-i.org/Profiles/BasicProfile-1.1-2004-08-24.html R2735 says rpc/literal accessor 
					// elements should not be in a namespace
		...
		$this->debug("endpoint=$this->endpoint, soapAction=$soapAction, namespace=$namespace, style=$style, ".
					"use=$use, encodingStyle=$encodingStyle");
		$this->debug('SOAP message length=' . strlen($soapmsg) . ' contents (max 1000 bytes)=' . 
					substr($soapmsg, 0, 1000));
		// send
	...
	*/
	...
	function loadWSDL() {
		...
		$this->wsdl = new wsdl('',$this->proxyhost,$this->proxyport,$this->proxyusername,$this->proxypassword,
			$this->timeout,$this->response_timeout,$this->curl_options,$this->use_curl);
