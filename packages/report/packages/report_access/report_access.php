	...
	class	report_access_1_0_0{
		...
		*/
		function			delete( $id , $Options = ' 1 = 1' )
		{
			try
			{
				$this->EventManager->trigger_event( 'on_before_delete_report' , array( 'id' => $id ) );

				$id = $this->Security->get( $id , 'integer_list' );
				$this->Database->delete( $this->NativeTable , "( $this->AddLimitations ) AND id IN ( $id )" );
				$this->Database->commit();

				$this->EventManager->trigger_event( 'on_after_delete_report' , array( 'id' => $id ) );
			}
			catch( Exception $e )
			{
				$a = func_get_args();_throw_exception_object( __METHOD__ , $a , $e );
		...
		*/
		function			create( $Record )
		{
			try
			{
				...
				$EventManager->trigger_event( 'on_after_create_report' , array( 'id' => $id ) );
		...
		*/
		function			update( $id , $Record )
		{
			try
			{
				$id = $this->Security->get( $id , 'integer_list' );
				$Record = $this->SecurityParser->parse_parameters( 
					$Record , 'domain:string;comment:string' , 'allow_not_set'
				);
