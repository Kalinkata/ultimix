		...
		function			get_connection( $ForceReconnect = false )
		{
			try
			{
				if( $this->Connection === false || $ForceReconnect ===true )
				{
					...
					$DBConfigSet->load_config( dirname( __FILE__ ).'/conf/cf_mssql_database' );
					$this->Connection = $DBConfigSet->connect( $this );
					if( mssql_get_last_message() !== false )
					{
						throw( 
							new Exception( 
								'An error occured while setting connection to the database '.mssql_get_last_message()
							)
						);
					}
				}
