<?php
	...
	*/
	class	pmsg_access_1_0_0{
		...
		*
		*	@exception Exception An exceptionof this type is thrown.
		*
		*	@author Dodonov A.A.
		*/
		...
		*	@author Dodonov A.A.
		*/
		function			delete( $id , $Condition = ' 1 = 1' )
		{
			try
			{
				$id = $this->Security->get( $id , 'integer_list' );
				$this->Database->delete( $this->NativeTable , "( $Condition ) AND id IN ( $id )" );
		...
		/**
		*	\~russian Получить логин админа.
		*
		*	@return Логин администратора, отвечающего за разбор жалоб пользователей.
		*
		*	@exception Exception Кидается исключение этого типа с описанием ошибки.
		*
		*	@author Додонов А.А.
		*/
		/**
		*	\~english Function return administrator's ligin.
		*
		*	@return Administrator's ligin who processes feedback messages.
		*
		*	@exception Exception An exceptionof this type is thrown.
		*
		*	@author Dodonov A.A.
		*/
		function			get_feedback_admin()
		{
			try
			{
				$Settings = get_package_object( 'settings::settings' , 'last' , __FILE__ );
				$Settings->load_file( dirname( __FILE__ ).'/conf/cf_pmsg_settings' );
				
				return( $Settings->get_setting( 'admin_login' , 'admin' ) );
			}
			catch( Exception $e )
			{
				$a = func_get_args();_throw_exception_object( __METHOD__ , $a , $e );
		...
		function			create( $Record )
		{
			try
			{
				$Record = $this->SecurityParser->parse_parameters( 
					$Record , 'author:string;recipient:string;subject:string;message:string'
				);
				...
				$EventManager->trigger_event( 'on_after_create_message' , array( 'id' => $id ) );
		...
		*/
		function			update( $id , $Record )
		{
			try
			{
				$id = $this->Security->get( $id , 'integer_list' );
				$Record = $this->SecurityParser->parse_parameters( 
					$Record , 'deleted:integer;recipient:string;author:string;subject:string;'.
						'message:string;`read`:integer' , 'allow_not_set'
				);
