...
/**
*	Function toggles 'view_in_report_field'.
*
...
*	@author Dodonov A.A.
*/
ultimix.page_text.toggle_view_in_report = function( TextId , Value )
{
	ultimix.ajax_gate.direct_controller( 
		{
			'package_name' : 'seo_tools::page_text_controller' , 'toggle_value' : Value , 
			'meta' : 'meta_toggle_view_in_report' , 'toggle_view_in_report' : 1 , 'text_id' : TextId
		} , 
		{} , 
		{}
	);
...
*	@author Dodonov A.A.
*/
ultimix.page_text.toggle_view_in_report_delegate = function( TextId )
{
	return(
		function( Value )
		{
			ultimix.page_text.toggle_view_in_report( TextId , Value );
		}
	);
}
...
if( !ultimix.lang.Strings[ ultimix.lang.Locale ][ 'view_in_report' ] ) ultimix.lang.Strings[ ultimix.lang.Locale ][ 'view_in_report' ] = {};
ultimix.lang.Strings[ ultimix.lang.Locale ][ 'view_in_report' ][ 'default' ] = 'Выводить в отчёте';
...
/***	Global namespace.**	@author Dodonov A.A.*/if( !ultimix ){	ultimix = {};}/***	Local namespace.**	@author Dodonov A.A.*/if( !ultimix.seo_tools ){	ultimix.seo_tools = {};}/***	Function runs requests processing.**	@param Query - Search query.**	@param Obj - Event emitted object.**	@param SearchEngine - Search engine.**	@author Dodonov A.A.*/ultimix.seo_tools.request_settings = function( Query , Obj , SearchEngine ){	return(		{ 			'package_name' : SearchEngine + '_proxy' , 'query' : Query , 'get_position' : 1 , 			'site' : jQuery( Obj ).parents( '.phrase_positions' ).find( '.site_input' ).val()		}	);}/***	Function creates requests processing.**	@param SearchEngine - Search engine name (yandex/google).**	@return Function.**	@author Dodonov A.A.*/ultimix.seo_tools.search_engine_proxy_request = function( SearchEngine ){	return(		function( Query , Obj , Iter )		{			ultimix.ajax_gate.direct_view( 				ultimix.seo_tools.request_settings( Query , Obj , SearchEngine ) , 				{					'success' : function( Result )					{						eval( 'Result = ' + Result + ';' );						jQuery( Iter.prev_element() ).next().html( Result.position ? Result.position : '-' );						Iter.next();					} 				} , 				{}			);		}	);}/***	Function adds grouping URL.**	@param Obj - Event emitted object.**	@param HasURLs - Were another urls already added.**	@param URL - URL to add.**	@return Tree brunch.**	@author Dodonov A.A.*/ultimix.seo_tools.add_url = function( Obj , HasURLs , URL ){	var			Last;	if( HasURLs )	{		Last = jQuery( Obj ).parents( '.phrase_positions' ).find( '.requests_grid' ).find( '.search_url' ).last();	}	else	{		Last = jQuery( Obj ).parents( '.phrase_positions' ).find( '.requests_grid' ).find( 'hr' ).last();	}	jQuery( Last ).after( 		'<div style="border: 1px solid black; margin: 5px 0 5px 0;" class="search_url" href="' + URL + 		'"><div style="width: 100%; display: block; background-color: #FFC7CE; font-weight: bold;">' + 		'<div style="height: 30px; line-height: 30px; padding: 3px; ">' + URL + '</div></div></div>'	);	return( jQuery( Last ).next() );}/***	Function adds grouping URL if necessary.**	@param Obj - Event emitted object.**	@param URL - URL to add.**	@return Tree brunch.**	@author Dodonov A.A.*/ultimix.seo_tools.add_unique_url = function( Obj , URL ){	var				URLs = jQuery( Obj ).parents( '.phrase_positions' ).find( '.requests_grid' ).find( '.search_url' );	for( var i = 0 ; i < URLs.length ; i++ )	{		if( jQuery( URLs[ i ] ).attr( 'href' ) == URL )		{			return( URLs[ i ] );		}	}	return( ultimix.seo_tools.add_url( Obj , URLs.length , URL ) );}/***	Function re-colors rows.**	@param Obj - Event emitted object.**	@author Dodonov A.A.*/ultimix.seo_tools.recolor_rows = function( Obj ){	var			URLs = jQuery( Obj ).parents( '.phrase_positions' ).find( '.requests_grid' ).find( '.search_url' );	for( var j = 0 ; j < URLs.length ; j++ )	{		var			Rows = jQuery( URLs[ j ] ).find( '.grid_row' );		for( var i = 0 ; i < Rows.length ; i++ )		{			jQuery( Rows[ i ] ).children( 'div' ).removeClass( 'table_row_even' );			jQuery( Rows[ i ] ).children( 'div' ).removeClass( 'table_row_odd' );			if( i % 2 )			{				jQuery( Rows[ i ] ).children( 'div' ).addClass( 'table_row_odd' );			}			else			{				jQuery( Rows[ i ] ).children( 'div' ).addClass( 'table_row_even' );			}		}	}}/***	Function moves brunch with the 'undefined' URL.**	@param Obj - Event emitted object.**	@author Dodonov A.A.*/ultimix.seo_tools.move_undefined = function( Obj ){	var				URLs = jQuery( Obj ).parents( '.phrase_positions' ).find( '.requests_grid' ).find( '.search_url' );	for( var i = 0 ; i < URLs.length ; i++ )	{		if( jQuery( URLs[ i ] ).attr( 'href' ) == 'undefined' )		{			if( i > 0 )			{				var			RequestsGrid = jQuery( Obj ).parents( '.phrase_positions' ).find( '.requests_grid' );				var			InsertElement = jQuery( RequestsGrid ).find( 'hr' ).last();				var			Brunch = jQuery( URLs[ i ] ).detach();				jQuery( InsertElement ).after( Brunch );			}		}	}}/***	AJAX request processor.**	@param Iter - Iterator.**	@param Obj - Event emitted object.**	@return Function.**	@author Dodonov A.A.*/ultimix.seo_tools.search_engine_proxy_request_and_group_success = function( Obj , Iter ){	return(		function( Result )		{			eval( 'Result = ' + Result + ';' );			var				Brunch = ultimix.seo_tools.add_unique_url( Obj , Result.url );			var				Element = jQuery( Iter.prev_element() ).parent().detach();			Element.find( 'div' ).eq( 1 ).html( Result.position ? Result.position : '-' );			Element.appendTo( Brunch );			ultimix.seo_tools.recolor_rows( Obj );			ultimix.seo_tools.move_undefined( Obj );			Iter.next();		}	);}/***	Function creates requests processing.**	@param SearchEngine - Search engine name (yandex/google).**	@author Dodonov A.A.*/ultimix.seo_tools.search_engine_proxy_request_and_group = function( SearchEngine ){	return(		function( Query , Obj , Iter )		{			ultimix.ajax_gate.direct_view( 				ultimix.seo_tools.request_settings( Query , Obj , SearchEngine ) , 				{					'success' : ultimix.seo_tools.search_engine_proxy_request_and_group_success( Obj , Iter )				} , 				{}			);		}	);}/***	Function finishes iteration throug phrases.**	@param Obj - Event emitted object.**	@return Function.**	@author Dodonov A.A.*/ultimix.seo_tools.phrases_end_iteration = function( Obj ){	return(		function()		{			jQuery( Obj ).parents( '.phrase_positions' ).removeClass( 'request_processing' )			jQuery( this.elements ).removeClass( 'search_query' );		}	);}/***	Function runs requests processing.**	@param Obj - Event emitted object.**	@param phrase_processor - Phrase processing function.**	@author Dodonov A.A.*/ultimix.seo_tools.start_phrase_processing = function( Obj , phrase_processor ){	if( jQuery( Obj ).parents( '.phrase_positions' ).hasClass( 'request_processing' ) == false )	{		jQuery( Obj ).parents( '.phrase_positions' ).addClass( 'request_processing' );		var				Iter = new ultimix.iterator( 			'.search_query' , 			function()			{				var			Query = jQuery( this.current_element() ).html();				jQuery( this.current_element() ).next().html( ultimix.get_string( 'processing' ) );				phrase_processor( Query , Obj , Iter );			} , 			ultimix.seo_tools.phrases_end_iteration( Obj )		);		Iter.next();	}}/***	Function adds row to grid.**	@param Grid - Grid.**	@param c - Counter.**	@param Data - Data of the first cell.**	@author Dodonov A.A.*/ultimix.seo_tools.add_row = function( Grid , c , Data ){	var			Style1 = "height: 30px; float: left; width: 400px; display: block; line-height: 30px;";	var			Style2 = "height: 30px; float: left; width: 100px; display: block; line-height: 30px;";	jQuery( Grid ).append( 		"<div style=\"height: 36px;\" class=\"grid_row\">" + 		"<div class=\"search_query " + ( c % 2 ? 'table_row_odd' : 'table_row_even' ) + 		"\"style=\"" + Style1 + "\">" + Data + "</div><div class=\"" + 		( c % 2 ? 'table_row_odd' : 'table_row_even' ) + " centered position\" style=\"" + Style2 + 			"\">&nbsp;</div>" + "</div>"	);}/***	Function adds requests to processing.**	@param Requests - Requests.**	@param Obj - Event emitted object.**	@author Dodonov A.A.*/ultimix.seo_tools.add_requests = function( Requests , Obj ){	if( Requests.length == 0 )	{		ultimix.std_dialogs.ErrorMessageBox( 'no_queries_specified' );		return;	}	jQuery( Obj ).parents( '.phrase_positions' ).find( '.requests_grid' ).show();	var			c = jQuery( Obj ).parents( '.phrase_positions' ).find( '.requests_grid' ).children( 'div' ).length;	for( var i = 0 ; i < Requests.length ; i++ , c++ )	{		ultimix.seo_tools.add_row( 			jQuery( Obj ).parents( '.phrase_positions' ).find( '.requests_grid' ) , c , Requests[ i ]		);	}	jQuery( Obj ).parents( '.phrase_positions' ).find( '.requests_input' ).val( '' );}/***	Function starts prepares processing.**	@param Obj - Event emitted object.**	@author Dodonov A.A.*/ultimix.seo_tools.prepare_data_for_processing = function( Obj ){	if( jQuery( Obj ).parents( '.phrase_positions' ).find( '.requests_input' ).val() == '' )	{		ultimix.std_dialogs.ErrorMessageBox( 'no_site_specified' );		return;	}	var			Test , Requests = jQuery( Obj ).parents( '.phrase_positions' ).find( '.requests_input' ).val();	var			Requests2 = [] , Request;	Requests = Requests.split( "\n" );	for( var i = 0 ; i < Requests.length ; i++ )	{		Requests[ i ] = ultimix.string_utilities.str_replace( "\r" , "" , Requests[ i ] );		Test = ultimix.string_utilities.str_replace( " " , "" , Requests[ i ] );		if( Test.length )		{			Requests2.push( Requests[ i ] );		}	}	ultimix.seo_tools.add_requests( Requests2 , Obj );}/***	Function starts requests processing.**	@param Obj - Event emitted object.**	@author Dodonov A.A.*/ultimix.seo_tools.add_strings_to_yandex_processor = function( Obj ){	ultimix.seo_tools.prepare_data_for_processing( Obj );	ultimix.seo_tools.start_phrase_processing( 		Obj , ultimix.seo_tools.search_engine_proxy_request( 'yandex' )	);}/***	Function starts requests processing.**	@param Obj - Event emitted object.**	@author Dodonov A.A.*/ultimix.seo_tools.add_strings_to_google_processor = function( Obj ){	ultimix.seo_tools.prepare_data_for_processing( Obj );	ultimix.seo_tools.start_phrase_processing( 		Obj , ultimix.seo_tools.search_engine_proxy_request( 'google' )	);}/***	Function starts requests processing.**	@param Obj - Event emitted object.**	@author Dodonov A.A.*/ultimix.seo_tools.add_strings_to_yandex_group_processor = function( Obj ){	ultimix.seo_tools.prepare_data_for_processing( Obj );	ultimix.seo_tools.start_phrase_processing( 		Obj , ultimix.seo_tools.search_engine_proxy_request_and_group( 'yandex' )	);}/***	Function starts requests processing.**	@param Obj - Event emitted object.**	@author Dodonov A.A.*/ultimix.seo_tools.add_strings_to_google_group_processor = function( Obj ){	ultimix.seo_tools.prepare_data_for_processing( Obj );	ultimix.seo_tools.start_phrase_processing( 		Obj , ultimix.seo_tools.search_engine_proxy_request_and_group( 'google' )	);}/***	Function clears request list.**	@param Obj - Event emitted object.**	@author Dodonov A.A.*/ultimix.seo_tools.clear_phrases = function( Obj ){	jQuery( Obj ).parents( '.phrase_positions' ).find( '.requests_grid' ).children( 'div' ).remove();	jQuery( Obj ).parents( '.phrase_positions' ).find( '.requests_grid' ).hide();	jQuery( Obj ).parents( '.phrase_positions' ).removeClass( 'request_processing' );}
if( !ultimix.lang.Strings[ ultimix.lang.Locale ][ 'seo_tools' ] ) ultimix.lang.Strings[ ultimix.lang.Locale ][ 'seo_tools' ] = {};
ultimix.lang.Strings[ ultimix.lang.Locale ][ 'seo_tools' ][ 'default' ] = 'Seo Tools';
if( !ultimix.lang.Strings[ ultimix.lang.Locale ][ 'system_buttons' ] ) ultimix.lang.Strings[ ultimix.lang.Locale ][ 'system_buttons' ] = {};
ultimix.lang.Strings[ ultimix.lang.Locale ][ 'system_buttons' ][ 'default' ] = 'Системные';
if( !ultimix.lang.Strings[ ultimix.lang.Locale ][ 'common_buttons' ] ) ultimix.lang.Strings[ ultimix.lang.Locale ][ 'common_buttons' ] = {};
ultimix.lang.Strings[ ultimix.lang.Locale ][ 'common_buttons' ][ 'default' ] = 'Общие';
if( !ultimix.lang.Strings[ ultimix.lang.Locale ][ 'sites' ] ) ultimix.lang.Strings[ ultimix.lang.Locale ][ 'sites' ] = {};
ultimix.lang.Strings[ ultimix.lang.Locale ][ 'sites' ][ 'default' ] = 'Сайты';
if( !ultimix.lang.Strings[ ultimix.lang.Locale ][ 'start_page' ] ) ultimix.lang.Strings[ ultimix.lang.Locale ][ 'start_page' ] = {};
ultimix.lang.Strings[ ultimix.lang.Locale ][ 'start_page' ][ 'default' ] = 'Index';
if( !ultimix.lang.Strings[ ultimix.lang.Locale ][ 'site_pages' ] ) ultimix.lang.Strings[ ultimix.lang.Locale ][ 'site_pages' ] = {};
ultimix.lang.Strings[ ultimix.lang.Locale ][ 'site_pages' ][ 'default' ] = 'Страницы сайта';
if( !ultimix.lang.Strings[ ultimix.lang.Locale ][ 'yandex_buttons' ] ) ultimix.lang.Strings[ ultimix.lang.Locale ][ 'yandex_buttons' ] = {};
ultimix.lang.Strings[ ultimix.lang.Locale ][ 'yandex_buttons' ][ 'default' ] = 'Яндекс';
if( !ultimix.lang.Strings[ ultimix.lang.Locale ][ 'google_buttons' ] ) ultimix.lang.Strings[ ultimix.lang.Locale ][ 'google_buttons' ] = {};
ultimix.lang.Strings[ ultimix.lang.Locale ][ 'google_buttons' ][ 'default' ] = 'Google';
if( !ultimix.lang.Strings[ ultimix.lang.Locale ][ 'yandex_positions' ] ) ultimix.lang.Strings[ ultimix.lang.Locale ][ 'yandex_positions' ] = {};
ultimix.lang.Strings[ ultimix.lang.Locale ][ 'yandex_positions' ][ 'default' ] = 'Позиции в Яндексе';
if( !ultimix.lang.Strings[ ultimix.lang.Locale ][ 'google_positions' ] ) ultimix.lang.Strings[ ultimix.lang.Locale ][ 'google_positions' ] = {};
ultimix.lang.Strings[ ultimix.lang.Locale ][ 'google_positions' ][ 'default' ] = 'Позиции в Google';
if( !ultimix.lang.Strings[ ultimix.lang.Locale ][ 'search_engine_requests' ] ) ultimix.lang.Strings[ ultimix.lang.Locale ][ 'search_engine_requests' ] = {};
ultimix.lang.Strings[ ultimix.lang.Locale ][ 'search_engine_requests' ][ 'default' ] = 'Результат обработки';
if( !ultimix.lang.Strings[ ultimix.lang.Locale ][ 'requests_input' ] ) ultimix.lang.Strings[ ultimix.lang.Locale ][ 'requests_input' ] = {};
ultimix.lang.Strings[ ultimix.lang.Locale ][ 'requests_input' ][ 'default' ] = 'Поисковые запросы';
if( !ultimix.lang.Strings[ ultimix.lang.Locale ][ 'process_requests' ] ) ultimix.lang.Strings[ ultimix.lang.Locale ][ 'process_requests' ] = {};
ultimix.lang.Strings[ ultimix.lang.Locale ][ 'process_requests' ][ 'default' ] = 'обработать запросы';
if( !ultimix.lang.Strings[ ultimix.lang.Locale ][ 'clear' ] ) ultimix.lang.Strings[ ultimix.lang.Locale ][ 'clear' ] = {};
ultimix.lang.Strings[ ultimix.lang.Locale ][ 'clear' ][ 'default' ] = 'очистить';
if( !ultimix.lang.Strings[ ultimix.lang.Locale ][ 'processing' ] ) ultimix.lang.Strings[ ultimix.lang.Locale ][ 'processing' ] = {};
ultimix.lang.Strings[ ultimix.lang.Locale ][ 'processing' ][ 'default' ] = 'обработка...';
if( !ultimix.lang.Strings[ ultimix.lang.Locale ][ 'no_site_specified' ] ) ultimix.lang.Strings[ ultimix.lang.Locale ][ 'no_site_specified' ] = {};
ultimix.lang.Strings[ ultimix.lang.Locale ][ 'no_site_specified' ][ 'default' ] = 'Введите пожалуйста сайт';
if( !ultimix.lang.Strings[ ultimix.lang.Locale ][ 'no_queries_specified' ] ) ultimix.lang.Strings[ ultimix.lang.Locale ][ 'no_queries_specified' ] = {};
ultimix.lang.Strings[ ultimix.lang.Locale ][ 'no_queries_specified' ][ 'default' ] = 'Введите пожалуйста хотя бы один поисковый запрос';
if( !ultimix.lang.Strings[ ultimix.lang.Locale ][ 'yandex_positions_group' ] ) ultimix.lang.Strings[ ultimix.lang.Locale ][ 'yandex_positions_group' ] = {};
ultimix.lang.Strings[ ultimix.lang.Locale ][ 'yandex_positions_group' ][ 'default' ] = 'Группировка анкеров';
if( !ultimix.lang.Strings[ ultimix.lang.Locale ][ 'google_positions_group' ] ) ultimix.lang.Strings[ ultimix.lang.Locale ][ 'google_positions_group' ] = {};
ultimix.lang.Strings[ ultimix.lang.Locale ][ 'google_positions_group' ][ 'default' ] = 'Группировка анкеров';
