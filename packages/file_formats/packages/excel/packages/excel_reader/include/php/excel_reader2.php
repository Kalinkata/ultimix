	...
	*/
	class Spreadsheet_Excel_Reader {
		...
		function dumpHexData($data, $pos, $length) {
			$info = "";
			for ($i = 0; $i <= $length; $i++) {
				$info .= ($i==0?"":" ") . $this->myHex(ord($data[$pos + $i])) . (ord($data[$pos + $i])>31? "[" . 
						 $data[$pos + $i] . "]":'');
			}
			return $info;
		}
			...
			for($row=1;$row<=$this->rowcount($sheet);$row++) {
				...
				for($col=1;$col<=$this->colcount($sheet);$col++) {
					...
					if(!$this->sheets[$sheet]['cellsInfo'][$row][$col]['dontprint']) {
						...
						$out .= "\n\t\t<td style=\"$style\"" . ($colspan > 1?" colspan=$colspan":"") . 
				...
				switch ($code) {
					...
					case SPREADSHEET_EXCEL_READER_TYPE_PALETTE:
							...
							for ($coli = 0; $coli < $colors; $coli++) {
								...
								$this->colors[0x07 + $coli] = '#' . $this->myhex($colr) . $this->myhex($colg) . 
															  $this->myhex($colb);
							}
							break;
	...
	//						$bgcolor = ($bgi & 0x3f80) >> 7;
							...
							$border = ord($data[$pos+14]) | (ord($data[$pos+15]) << 8) | (ord($data[$pos+16]) << 16) | 
									  (ord($data[$pos+17]) << 24);
									...
									if ($formatstr!="") {
										...
										if (preg_match("/[^hmsday\/\-:\s\\\,AMP]/i", $tmp) == 0) { 
											...
											$formatstr = str_replace(
												array('AM/PM','mmmm','mmm'), array('a','F','M'), $formatstr
											);
		...
		function _parsesheet($spos) {
				...
				switch ($code) {
					...
					case SPREADSHEET_EXCEL_READER_TYPE_FORMULA2:
						...
						if ((ord($data[$spos+6])==0) && (ord($data[$spos+12])==255) && (ord($data[$spos+13])==255)) {
							...
							$previousRow = $row;
							$previousCol = $column;
						} elseif ((ord($data[$spos+6])==1) && (ord($data[$spos+12])==255) && 
						...
						} elseif ((ord($data[$spos+6])==2) && (ord($data[$spos+12])==255) && 
								  (ord($data[$spos+13])==255)) {
							//Error formula. Error code is in +2;
						} elseif ((ord($data[$spos+6])==3) && (ord($data[$spos+12])==255) && 
								  (ord($data[$spos+13])==255)) {
					...
					case SPREADSHEET_EXCEL_READER_TYPE_LABEL:
						...
						$column = ord($data[$spos+2]) | ord($data[$spos+3])<<8;
						$this->addcell(
							$row, $column, substr($data, $spos + 8, ord($data[$spos + 6]) | ord($data[$spos + 7])<<8)
						);
						break;
					...
					case SPREADSHEET_EXCEL_READER_TYPE_COLINFO:
						...
						for ($coli = $colfrom; $coli <= $colto; $coli++) {
							$this->colInfo[$this->sn][$coli+1] = Array(
								'width' => $cw, 'xf' => $cxf, 'hidden' => ($co & 0x01), 
			...
			function _getCellDetails($spos,$numValue,$column) {
				...
				if ($type == 'date') {
					...
					$utcDays = floor($numValue - ($this->nineteenFour ? SPREADSHEET_EXCEL_READER_UTCOFFSETDAYS1904 : 
					...
					$string = date (
						$format, mktime($hours, $mins, $secs, $dateinfo["mon"], $dateinfo["mday"], $dateinfo["year"])
					);
				} else if ($type == 'number') {
					$rectype = 'number';
			...
			if ($this->_defaultEncoding){
				switch ($this->_encoderFunction){
					case 'iconv' :	 $result = iconv('UTF-16LE', $this->_defaultEncoding, $string);
									break;
					case 'mb_convert_encoding' : $result = mb_convert_encoding(
						$string, $this->_defaultEncoding, 'UTF-16LE'
					);
									break;
				}
			}
